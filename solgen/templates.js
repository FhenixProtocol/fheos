...
function TypeCastTestingFunction(fromType, fromTypeForTs, toType, fromTypeEncrypted) {
    ...
    testType = testType === 'bytes memory' ? 'PreEncrypted' : (0, exports.capitalize)(testType);
    testType = testType === 'Uint256' ? 'Plaintext' : testType;
    var encryptedVal = fromTypeEncrypted ? "TFHE.as".concat((0, exports.capitalize)(fromTypeEncrypted), "(val)") : "val";
    var func = "\tfunction castFrom".concat(testType, "To").concat(to, "(").concat(fromType, " val) public pure returns (").concat(retType, ") {\n    \treturn TFHE.decrypt(TFHE.as").concat(to, "(").concat(encryptedVal, "));\n\t}\n\n");
    var retTypeTs = (retType === 'bool') ? 'boolean' : retType;
    retTypeTs = retTypeTs.includes("uint") ? 'bigint' : retTypeTs;
    var abi = "\tcastFrom".concat(testType, "To").concat(to, ": (val: ").concat(fromTypeForTs, ") => Promise<").concat(retTypeTs, ">;\n");
    return [func, abi];
}
function AsTypeTestingContract(type) {
    var funcs = "";
    var abi = "export interface As".concat((0, exports.capitalize)(type), "TestType extends Contract {\n");
    for (var _i = 0, _a = common_1.EInputType.concat('uint256', 'bytes memory'); _i < _a.length; _i++) {
        var fromType = _a[_i];
        if (type === fromType) {
            continue;
        }
        ...
        var fromTypeEncrypted = (common_1.EInputType.includes(fromType)) ? fromType : undefined;
...
function testContract2ArgBoolRes(name, isBoolean) {
    ...
    if (isBoolean) {
        func += " else if (Utils.cmp(test, \"".concat(name, "(ebool,ebool)\")) {\n            bool aBool = true;\n            bool bBool = true;\n            if (a == 0) {\n                aBool = false;\n            }\n            if (b == 0) {\n                bBool = false;\n            }\n            if (TFHE.decrypt(TFHE.").concat(name, "(TFHE.asEbool(aBool), TFHE.asEbool(bBool)))) {\n                return 1;\n            }\n\n            return 0;\n        }");
    }
