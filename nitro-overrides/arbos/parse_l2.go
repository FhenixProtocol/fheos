...
const (
	...
	L2MessageKind_SignedDecryptionTx = 9
)
...
func parseL2Message(rd io.Reader, poster common.Address, timestamp uint64, requestId *common.Hash, chainId *big.Int, depth int) (types.Transactions, error) {
	...
	case L2MessageKind_SignedDecryptionTx:
		// A signed decryption tx is of the form: decryptionResults | signedTx

		// Extract the decryption results
		err := fheos.LoadMultipleResolvedDecryptions(rd)
		if err != nil {
			return nil, err
		}

		// The rest of the message is a signed tx
		return parseL2Message(rd, poster, timestamp, requestId, chainId, depth)
	default:
