...
func MessageFromTxes(header *arbostypes.L1IncomingMessageHeader, txes types.Transactions, hooks *arbos.SequencingHooks) (*arbostypes.L1IncomingMessage, error) {
	...
	if len(txes) == 1 && txErrors[0] == nil {
		data, err := MessageFromSignedTx(txes[0], hooks)
		if err != nil || data == nil {
			return nil, err
		}
		l2Message = append(l2Message, data...)
	} else {
		l2Message = append(l2Message, arbos.L2MessageKind_Batch)
		txSizeBuf := make([]byte, 8)
		for i, tx := range txes {
			if txErrors[i] != nil {
				continue
			}
			...
			// prepend transaction length
			binary.BigEndian.PutUint64(txSizeBuf, uint64(len(txBytes)))
			l2Message = append(l2Message, txSizeBuf...)
...
// MessageFromSignedTx returns a byte-serialization of a transaction.
func MessageFromSignedTx(tx *types.Transaction, hooks *arbos.SequencingHooks) ([]byte, error) {
	// The structure the encoded message is:
	// if there are decryption results associated with the transaction:
	//	encoded_async_tx = L2MessageKind_AsyncSignedTx | async_results | L2MessageKind_AsyncSignedTx | tx_data
	// Note: see SerializeTxDecryptRes for the formatting of async_results
	// if there are no decryption results associated with the transaction:
	//	encoded_async_tx = L2MessageKind_AsyncSignedTx | tx_data
	var data []byte

	decryptResults, err := hooks.SerializeTxDecryptRes(tx)
	if err != nil {
		log.Error("error during serialization of decryption results", "err", err)
		return nil, err
	}
	...
	data = append(data, arbos.L2MessageKind_SignedTx)
...
func (s *ExecutionEngine) sequenceTransactionsWithBlockMutex(header *arbostypes.L1IncomingMessageHeader, txes types.Transactions, hooks *arbos.SequencingHooks) (*types.Block, error) {
	...
	msg, err := MessageFromTxes(header, txes, hooks)
	if err != nil {
		return nil, err
	}
